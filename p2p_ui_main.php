...
function p2pui_connection_meta_content( $post ) {
	...
	<input type="text" id="p2pui_connection_name" name="p2pui_connection_name"
	       value="<?php echo get_post_meta( $post->ID, 'p2pui_connection_name', true ); ?>"/><br/>
	...
	<select id="p2pui_cardinality" name="p2pui_cardinality">
		...
		foreach ( $p2pui_cardinality as $cardinality_key => $cardinality_value ):
			if ( $saved_cardinality === $cardinality_key ) {
				?>
				<option value="<?php echo $cardinality_key; ?>"
				        selected="selected"><?php echo $cardinality_value; ?></option>
			<?php } else { ?>
	...
	foreach ( $connections_to_register as $post ) :
		...
		if ( ! empty( $name ) && ! empty( $from ) && ! empty( $to ) ) {
			...
			p2p_register_connection_type( array(        //function from the Posts-to-Posts plugin that actually registers the connection types
				...
				'fields'      => p2pui_get_connection_fields( $post ),
				//see below
